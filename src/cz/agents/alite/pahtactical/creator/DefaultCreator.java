...
import cz.agents.alite.tactical.vis3d.layer.Car3dLayer;
...
public abstract class DefaultCreator implements Creator {
    ...
    protected static final Logger LOGGER = Logger.getLogger(DefaultCreator.class);
    protected static final long RANDOM_SEED = 0;
    protected PAHUniverse universe;
    ...
    public void create(long simulationEndTime, double simulationSpeed) {
        createCommons();

        createSimulation(simulationEndTime, simulationSpeed);
        createEnvironment();
        createVisualization();
        createEntities();
    ...
    protected void createSimulation(long simulationEndTime, double speed) {
        LOGGER.info(">>> SIMULATION CREATION");

        if (simulationEndTime == 0) {
            simulation = new Simulation();
        } else {
            simulation = new Simulation(simulationEndTime);
        }
    ...
    protected PAHCarAgent addCarAgent(Point initialLocation, Set<Point> checkpoints, double checkpointReachedTolerance, Car.VehicleType vehicleType) {
        ...
        carAgent.setCheckpointReachedTolerance(checkpointReachedTolerance);
        universe.addCustomCarEntity("car" + carCounter, carAgent, Team.ALLY, initialLocation, vehicleType);
        carCounter++;
        return carAgent;
    }
    ...
    protected void create3dVisualization() {
        ...
        Vis3dManager.registerLayer(Car3dLayer.create(universe.getEnvironment().getCarStorage(), universe.getPhysics()));
